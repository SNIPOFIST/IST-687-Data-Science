...
colnames(static_house_data)

head(static_house_data$in.sqft)

unique(static_house_data$in.hvac_cooling_efficiency)
...
energy_data <- read_parquet("/Users/saranya/Desktop/Classroom/Introduction to Data Science /IDS Project/COMBINED_ENERGY_DATASET_5000_House_IDS.parquet copy")
dim(data)

colnames(energy_data)


# Select columns that start with "out.electricity", along with "time" and "building_id"
new_data <- energy_data %>% select(starts_with("out.electricity"),time,building_id) # Matches all columns starting with "out.electricity"time,                           # 
...
write_parquet(new_data,"/Users/saranya/Desktop/Classroom/Introduction to Data Science /IDS Project/ filtered_july_data.parquet")
...
library(arrow)
final_merged_data <- data.frame(read_parquet("/Users/saranya/Desktop/Classroom/Introduction to Data Science /IDS Project/ DATA.parquet"))
```
...
```{r}
write_parquet(final_merged_data, "/Users/saranya/Desktop/Classroom/Introduction to Data Science /IDS Project/ DATA.parquet")
```
...
linear_model_r1 <- lm(
  electricity ~ building_id + in.geometry_floor_area + in.geometry_stories + in.vintage + 
...
# Plotting electricity against building_id
ggplot(final_merged_data, aes(x = as.factor(building_id), y = electricity)) +
  geom_boxplot() + 
  labs(
    title = "Electricity Consumption by Building ID",
    x = "Building ID",
...
# Subset the data to include only the independent variables from the model
independent_vars <- final_merged_data %>%
  select(
    building_id, 
...
# Glimpse of the independent variables
glimpse(independent_vars)

library(ggplot2)

# Create a scatter plot to visualize electricity consumption vs. floor area
ggplot(final_merged_data, aes(x = in.geometry_floor_area, y = electricity)) +
  geom_point() +  # Scatter plot
  labs(
    title = "Electricity Consumption vs. Floor Area",
    x = "Floor Area (sqft)",
    y = "Electricity Consumption"
  ) +
...
ggplot(final_merged_data, aes(x = electricity)) +
  ...
  labs(title = "Density Plot of Electricity Consumption", x = "Electricity Consumption") +
  theme_minimal()
...
x <- as.matrix(final_merged_data[, .(
  building_id, in.geometry_floor_area, in.geometry_stories, in.vintage,
...
# Scatter plots for electricity vs. independent variables
ggplot(final_merged_data, aes(x = in.cooling_setpoint, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Electricity vs. Cooling Setpoint", x = "Cooling Setpoint", y = "Electricity Consumption")


library(ggplot2)
library(patchwork)

# Plot 1: Electricity vs. Cooling Setpoint
plot1 <- ggplot(final_merged_data, aes(x = in.cooling_setpoint, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Electricity vs. Cooling Setpoint", x = "Cooling Setpoint", y = "Electricity Consumption")

# Plot 2: Electricity vs. Heating Setpoint
plot2 <- ggplot(final_merged_data, aes(x = in.heating_setpoint, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Electricity vs. Heating Setpoint", x = "Heating Setpoint", y = "Electricity Consumption")

# Plot 3: Electricity vs. Number of Occupants
plot3 <- ggplot(final_merged_data, aes(x = in.occupants, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Electricity vs. Number of Occupants", x = "Number of Occupants", y = "Electricity Consumption")

# Combine the three plots into one plot using patchwork
combined_plot <- plot1 + plot2 + plot3 + 
  plot_layout(ncol = 3)  # arrange them in 1 row and 3 columns
...
# remove the insignificant value : in.misc_gas_fireplace

# Linear Regression
linear_model_behavioural <- lm(
  electricity ~ in.cooling_setpoint + in.heating_setpoint + in.occupants +
    in.lighting + in.misc_extra_refrigerator + in.misc_pool_pump + in.range_spot_vent_hour +
    in.hot_water_fixtures + in.vacancy_status,
  data = final_merged_data
)
...
# Plot for dry_bulb_temperature vs electricity
plot_dry_bulb <- ggplot(final_merged_data, aes(x = dry_bulb_temperature, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Dry Bulb Temperature vs Electricity", x = "Dry Bulb Temperature", y = "Electricity")


plot_dry_bul

# Plot for relative_humidity vs electricity
plot_relative_humidity <- ggplot(final_merged_data, aes(x = relative_humidity, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Relative Humidity vs Electricity", x = "Relative Humidity", y = "Electricity")
...
library(patchwork)
combined_plot <- (plot_dry_bulb | plot_relative_humidity)
combined_plot
...
# Create a subset with the variables used in the model
model_data <- final_merged_data[, c(
  "out.electricity.cooling.energy_consumption",
  "out.electricity.heating.energy_consumption",
  "out.electricity.lighting_interior.energy_consumption",
  "out.electricity.refrigerator.energy_consumption",
...
# Plot for out.electricity.cooling.energy_consumption vs electricity
plot_cooling <- ggplot(final_merged_data, aes(x = out.electricity.cooling.energy_consumption, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Cooling Energy Consumption vs Electricity", x = "Cooling Energy Consumption", y = "Electricity")

plot_cooling

# Plot for out.electricity.heating.energy_consumption vs electricity
plot_heating <- ggplot(final_merged_data, aes(x = out.electricity.heating.energy_consumption, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Heating Energy Consumption vs Electricity", x = "Heating Energy Consumption", y = "Electricity")
plot_heating

# Plot for out.electricity.lighting_interior.energy_consumption vs electricity
plot_lighting <- ggplot(final_merged_data, aes(x = out.electricity.lighting_interior.energy_consumption, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Lighting Energy Consumption vs Electricity", x = "Lighting Energy Consumption", y = "Electricity")

plot_lighting

# Plot for out.electricity.refrigerator.energy_consumption vs electricity
plot_refrigerator <- ggplot(final_merged_data, aes(x = out.electricity.refrigerator.energy_consumption, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Refrigerator Energy Consumption vs Electricity", x = "Refrigerator Energy Consumption", y = "Electricity")
...
# Plot for in.hvac_cooling_efficiency vs electricity
plot_hvac_cooling <- ggplot(final_merged_data, aes(x = in.hvac_cooling_efficiency, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "HVAC Cooling Efficiency vs Electricity", x = "HVAC Cooling Efficiency", y = "Electricity")
plot_hvac_cooling

# Plot for relative_humidity vs electricity
plot_humidity <- ggplot(final_merged_data, aes(x = relative_humidity, y = electricity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  theme_minimal() +
  labs(title = "Relative Humidity vs Electricity", x = "Relative Humidity", y = "Electricity")

# Combine the plots into one using patchwork (adjust the layout as needed)
combined_plots <- (plot_cooling | plot_heating | plot_lighting) /
                  (plot_refrigerator | plot_dry_bulb | plot_hvac_cooling) /
                  (plot_humidity)
...
# Plotting the relationship between in.geometry_floor_area and electricity
plot_floor_area <- ggplot(final_merged_data, aes(x = in.geometry_floor_area, y = electricity)) +
  geom_point(color = "blue") +  # scatter plot
  geom_smooth(method = "lm", color = "red") +  # regression line
  theme_minimal() +
  labs(title = "Floor Area vs Electricity Consumption",
       x = "Floor Area (in square meters)", y = "Electricity Consumption")

# Display the plot
print(plot_floor_area)
...
# Plotting for Insulation Ceiling
plot_ceiling <- ggplot(final_merged_data, aes(x = as.factor(in.insulation_ceiling), y = electricity, fill = as.factor(in.insulation_ceiling))) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge") +
  theme_minimal() +
  labs(title = "Mean Electricity Consumption by Insulation Ceiling", 
       x = "Insulation Ceiling", y = "Mean Electricity Consumption") +
  scale_fill_brewer(palette = "Set3")

# Plotting for Insulation Floor
plot_floor <- ggplot(final_merged_data, aes(x = as.factor(in.insulation_floor), y = electricity, fill = as.factor(in.insulation_floor))) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge") +
  theme_minimal() +
  labs(title = "Mean Electricity Consumption by Insulation Floor", 
       x = "Insulation Floor", y = "Mean Electricity Consumption") +
  scale_fill_brewer(palette = "Set3")

# Plotting for Insulation Wall
plot_wall <- ggplot(final_merged_data, aes(x = as.factor(in.insulation_wall), y = electricity, fill = as.factor(in.insulation_wall))) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge") +
  theme_minimal() +
  labs(title = "Mean Electricity Consumption by Insulation Wall", 
       x = "Insulation Wall", y = "Mean Electricity Consumption") +
  scale_fill_brewer(palette = "Set3")
...
# Save necessary results for Shiny app
save_results_for_shiny <- function() {
  # Load required libraries
  library(xgboost)
  library(data.table)

  # Define necessary objects to save
  results <- list(
    # Research Question 1: Building Attributes
    linear_model_r1_summary = summary(linear_model_r1),
    xgb_model_r1 = xgb_model_r1,
    
    # Predictions and evaluations
    linear_preds_r1 = linear_preds_r1,
    linear_results_r1 = linear_results_r1,

    # Research Question 2: Behavioral Patterns
    linear_model_behavioural_summary = summary(linear_model_behavioural),
    xgb_model_behavioural = xgb_model_behavioural,
    linear_preds_behavioural = linear_preds_behavioural,
    linear_results_behavioural = linear_results_behavioural,

    # Research Question 3: Weather Attributes
    linear_model_r3_summary = summary(linear_model_r3),
    xgb_model_r3 = xgboost_model_r3,
    results_r3models = results_r3models,

    # Research Question 4: Energy Efficiency
    linear_model_energy_efficient_summary = summary(linear_model_energy_efficient),
    predictions_energy_efficient = predictions,
    r_squared_energy_efficient = r_squared,
    rmse_energy_efficient = rmse,

    # Final Model
    final_model_linear_summary = summary(linear_model),
    final_model_xgb = xgb_model,

    # Precomputed data for final model's correlation matrix (optional)
    correlation_matrix_final_model = cor(data, use = "complete.obs"),

    # Precomputed datasets (if needed in the app)
    test_data = test_data,
    train_data = train_data
  )

  # Save all results into an RData file
  save(results, file = "precomputed_results.RData")

  cat("Results saved successfully to precomputed_results.RData\n")
}

# Run the function
save_results_for_shiny()
...
warmjuly_weatherdata <- final_merged_data %>% 
...
model_data <- model.frame(linear_model_behavioural)

# Create data frame for fitted vs actual
plot_data <- data.frame(
  Fitted = fitted(linear_model_behavioural),
  Actual = model_data$electricity
)

# Plot Fitted vs Actual
ggplot(plot_data, aes(x = Fitted, y = Actual)) +
  geom_point(color = "blue", alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(
    title = "Fitted vs. Actual Electricity Consumption",
    x = "Fitted Values (Predicted Electricity)",
    y = "Actual Electricity Consumption"
  ) +
  theme_minimal()

# Create data frame for residual plot
residual_data <- data.frame(
  Fitted = fitted(linear_model_behavioural),
  Residuals = resid(linear_model_behavioural)
)
